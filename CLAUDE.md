...
- **`load()` must persist its own `applyDayReset()` result to `localStorage` immediately** (fixed 2026-09-24), not leave the reset in memory only. `mergeCloudState()` re-reads `localStorage` fresh on every call (it doesn't trust the in-memory `state`), so a reset that only exists in memory is invisible to it — it'll see yesterday's un-reset `editedToday`/`lastReset` and, if no device has saved today's reset to Firebase yet either, its same-day merge logic (`Math.max` of local vs cloud, gated on both sides agreeing it's "today") can treat those stale numbers as legitimate and carry them forward, which throws off anything downstream that expects `p.edited` to reflect actually-completed work (e.g. the work-plan's `unplannedRemaining`).
